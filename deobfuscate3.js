...
traverse(ast, {
  VariableDeclaration(path) {
    ...
    if (
      ...
      !init.callee ||
      !init.callee.callee ||
      !init.callee.callee.name
    ) {
      return;
    }
    console.log("!!!");

    const binding = scope.getBinding(init.callee.callee.name);
    // console.log("bining", binding);
    if (!binding || !binding.path) {
      return;
    }
    console.log("binind", binding.path);
    const key = init.callee.arguments[0].value;
    const functionExpression = binding.path.node.init.properties.filter(
      (o) => o.key.name == key
    );
    console.log("functionExpression", functionExpression);
    const code = generate(functionExpression[0]);
